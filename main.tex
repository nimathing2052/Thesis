...
\usepackage[style=apa, backend=biber, url=true, natbib=true]{biblatex} % Removed natbib package, added natbib=true for compatibility
...
\fancyhf{}
\rhead{\thepage}
\lhead{TikTok Political Virality Thesis}
...
% This study investigates the predictability of political virality on TikTok during Nepal’s 2022 local elections (RQ1) and the interaction of communication styles and political content themes in shaping engagement (RQ2). The findings reveal the complex interplay of content, platform, and sender characteristics, offering insights into the dynamics of political communication in a youth-dominated, algorithm-driven social media ecosystem. 

% \subsection{Multimodal Dynamics of Virality}
% The SHAP analysis (Figure~\ref{fig:shap_summary}) revealed that no single modality dominated, underscoring the complex, multifaceted nature of TikTok engagement during Nepal’s election period. Sender characteristics emerged as the most predictive, with \texttt{user\_like\_count} (mean $|$SHAP$|$ $\approx$ 0.8) and \texttt{Video\_count} (mean $|$SHAP$|$ $\approx$ 0.5) strongly influencing high-virality predictions (Class 2). Content embeddings also played a significant role, with \texttt{audio\_pca\_0} (mean $|$SHAP$|$ $\approx$ 0.3) impacting medium-virality predictions (Class 1) and \texttt{image\_pca\_2} (mean $|$SHAP$|$ $\approx$ 0.1) contributing to high virality, as observed in the local explanation of video ID 7093702566613110043 (Figure~\ref{fig:shap_force_plot}). This video, a humorous commentary on a news story involving Former Finance Minister Janardan Sharma, leveraged a news broadcast scene with an airplane inset and a comedic audio transcript, aligning with high-engagement patterns.

% Audio embeddings captured Nepal’s oral tradition (e.g., humorous, voiceovers with comedy, movie/dilaogues), while image embeddings reflected political imagery (appeal for charismatic leader), aligning with multimodal engagement studies \citep{highfield2016instagrammatics} (also discussed in RQ2). The influence of video_diversification_id_10083.0 (critical political speech, humor) suggests thematic content drives high virality, but its sparsity necessitates future mapping to semantic categories.

% \subsection{Sender Influence and Algorithmic Bias}
% The SHAP analysis underscored the outsized influence of sender characteristics, with \texttt{user\_like\_count}, \texttt{Video\_count}, and \texttt{followers} (mean $|$SHAP$|$ $\approx$ 0.4) among the top predictors of high virality. This reflects TikTok’s algorithmic bias toward amplifying content from creators with established visibility, a well-documented phenomenon in platform studies \parencite{bucher2018ifthen}. In Nepal’s 2022 election context, this dynamic favored prominent figures, such as independent candidate Balen Shah, who effectively leveraged social media platform such as Facebook to get traction (and ifnluential page such as Routine of Nepal Banda (currently 3M+ followers) heavily promoted him \parencite{Kandel2024}, while also had a substantial following in the past. It's effect was also seen in corollary with TikTok, which has a good base in Nepal but as not strong as Facebook \parencite{Onlinekhabar2022}. The error analysis illustrated this bias in Case 1 (ID: 7095593756967046426), where a medium-viral video was overpredicted as high-viral due to a high \texttt{user\_like\_count} (19,854), highlighting the model’s tendency to prioritize creator engagement over actual video performance. This bias may marginalize lesser-known candidates, raising equity concerns \citep{noble2018algorithms}. Error analysis showed underprediction of high-viral videos with strong engagement (Case 3, ID: 7089029999780498715: 2,820,140 views, 161,239 likes), possibly due to content features favoring medium virality, highlighting the need for balanced engagement metrics.
...
% Nuanced factors like cultural authenticity and hyper-local sentiment remain challenging to quantify. The low impact of content embeddings (mean SHAP: audio 0.0013, image 0.0010) suggests PCA reduction limited semantic capture, as seen in the local explanation where desc_pca_0 had a negative impact. Frequent misclassifications between low and medium virality (62\% of Class 1 errors as Class 0) reflect overlapping engagement distributions, limiting precision for Class 1.


% \section{Style--Theme Interactions in Political Communication (RQ2)}
% \label{sec:rq2_discussion}
% The OLS regression analysis (see Appendix~\ref{tab:ols_virality_summary}) revealed that communication styles and political content themes interact in context-dependent ways to shape TikTok virality, with significant implications for political messaging strategies.

% \subsection{Style Effects: Humor and Civic Engagement}
% Comedic (\( \beta = 1.87 \), \( p = 0.003 \)) and Civic Awareness (\( \beta = 1.19 \), \( p = 0.037 \)) styles were strongly associated with virality, aligning with TikTok’s entertainment-driven platform logic \citep{zulli2020tiktok}. Humor’s effectiveness reflects its ability to capture attention in a crowded content ecosystem, while Civic Awareness resonates with youth audiences seeking educational yet accessible political content. However, these styles’ impact varied significantly by political theme.

% \subsection{Independent Candidates: Virality and Authenticity}
% Content about independent candidates, particularly figures like Balen Shah, exhibited higher virality (\( \beta = 1.06 \), \( p = 0.028 \)) compared to general election-themed content. This reflects their appeal among Nepal’s youth, who often view independents as anti-establishment alternatives to legacy parties. However, a significant negative interaction between Comedic style and Independent candidate content (\( \beta = -2.56 \), \( p < 0.001 \)) suggests that humor may undermine perceived authenticity. Audiences appear to expect sincerity from independents, penalizing comedic framing as trivializing or inauthentic.

% \subsection{Legacy Parties: Challenges of Civic Messaging}
% Civic Awareness styles paired with Congress (\( \beta = -1.94 \), \( p = 0.007 \)) and UML (\( \beta = -1.47 \), \( p = 0.029 \)) content showed negative interactions, reducing virality. This may reflect youth skepticism toward legacy parties, perceived as entrenched and disconnected from grassroots concerns. TikTok’s young users may view didactic or educational tones from these parties as inauthentic, preferring performative or emotionally resonant content instead.

% The heatmap analysis (see Fig.~\ref{fig:TikTokComm}) revealed that legacy parties heavily relied on Musical/Entertainment styles (42--48\%), featuring campaign songs and symbolic imagery (e.g., the Tree for Congress). However, this stylistic saturation diluted their impact, rendering the style statistically non-significant in the regression model. In contrast, less common styles like Charisma/Leadership, prevalent among independent candidates (28.4\%), offered greater marginal returns, leveraging rap or hip-hop to blend cultural performance with political branding.

% \subsection{Cultural Strategies and Connection to RQ1}
% The style–theme distribution reflects deliberate framing: non-political satirical content used Comedic or Critique/Frustration styles, facilitating indirect political discourse through parody, while traditional parties leveraged cultural familiarity via folk music. These findings complement RQ1’s XGBoost results, where video_diversification_id_10083.0 (critical political speech, humor) predicted high virality, aligning with the OLS’s emphasis on Comedic style. Sender features like follower count were significant in both models, reinforcing TikTok’s bias toward established creators, though the OLS highlights content-specific dynamics (e.g., Independent candidates’ appeal) not captured by RQ1’s embeddings.

% \section{Broader Implications}
% \label{sec:implications}

% The findings across RQ1 and RQ2 offer three key insights for political communication in digital spaces:
% \begin{itemize}[noitemsep]
%     \item \textbf{Algorithmic Predictability}: Virality can be forecasted with modest accuracy, but biases toward popular creators limit equitable visibility, reinforcing existing power structures.
%     \item \textbf{Style as a Political Tool}: Communication styles actively shape engagement, with humor and charisma amplifying independent candidates’ appeal, while legacy parties struggle to overcome youth skepticism.
%     \item \textbf{Youth-Driven Discourse}: TikTok’s emphasis on authenticity, emotion, and performance creates opportunities for independent movements but risks amplifying performative populism or misinformation.
% \end{itemize}
% These dynamics are particularly salient in Nepal, where youth disillusionment with traditional politics fuels demand for novel, culturally resonant messaging. However, the findings have global relevance, as platforms like TikTok increasingly mediate political expression worldwide.
% \newpage
...
% \begin{itemize}[noitemsep]
%     \item The \texttt{video\_diversification\_id} variable lacks semantic clarity, introducing potential noise in predictive models.
%     \item Mixed-language transcripts and Romanized Nepali posed challenges for audio embeddings, limiting textual fidelity.
%     \item Class imbalance and the dataset’s focus on the 2022 election period may constrain generalizability.
% \end{itemize}

% Future research could address these by:
% \begin{itemize}[noitemsep]
%     \item Developing time-series models to capture virality dynamics over campaign cycles.
%     \item Comparing virality patterns across platforms (e.g., YouTube Shorts, Instagram Reels).
%     \item Conducting audience reception studies using comment analysis or surveys to explore user perceptions.
%     \item Employing advanced NLP techniques, such as multilingual embeddings or Fine-Tuned version of Nepali or Nepali-Romanized textual understanding to enhance audio and textual analysis.
% \end{itemize}

% By addressing these gaps, scholars can further illuminate the evolving role of social media in political communication, particularly in diverse, multilingual contexts like Nepal.
...
% Audio embeddings effectively captured Nepal's rich oral tradition, where campaign speeches and humorous voiceovers resonated strongly with audiences. On the other hand, image embeddings reflected political imagery, including visuals commonly seen in news broadcasts and public TV debates/fiery exchanges. For example, the controversial and popular TV show Dhamala Ko Hamala \parencite{Dorjee2022}, whose many clips with celebrities which became a subject of viral-memes/trends, aligned with broader multimodal engagement patterns \parencite{highfield2016instagrammatics}. Furthermore, platform-specific features, such as \texttt{diversification\_id\_10083.0} (mean $|$SHAP$|$ $\approx$ 0.1), which were linked to critical political speech and humor, played a significant role in boosting virality predictions.
